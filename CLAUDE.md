...
- **Model-size gap confirmed:** `torchtitan`'s shipped model configs are Llama 3.1 at 8B/70B/405B only — **no small (<1B) preset exists**. `configs/models/130m.toml` / `500m.toml` / `1b.toml` (already scaffolded, dims marked `[UNKNOWN]`) will need real values written against `torchtitan`'s actual `Config` dataclass field names on Day 0 — this was already known to be required (ADR-009), now confirmed as definitely required rather than possibly-avoidable.
...
*Decision:* **PENDING — exact pins are Day-0-time actions by design; this entry now has enough information to make that a fast, low-risk decision rather than a blind one.**
